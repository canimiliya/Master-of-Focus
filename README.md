# 专注王者（Study Game Pro）

一个结合番茄钟、任务清单、复盘与游戏兑换奖励的自律工具（Tkinter GUI）。

![主界面显示](主界面显示.png)

## 1. 依赖安装与运行命令

### 环境要求
- Windows 10/11
- Python 3.8+（推荐 3.10+）

### 安装依赖
本项目主要依赖标准库与以下第三方库：

```bash
pip install matplotlib numpy
```

可选功能依赖（不装也能运行，但建议安装）：

```bash
pip install tkinterdnd2 win10toast
```

> 说明：
> - `tkinterdnd2` 用于随手记的拖拽上传。
> - `win10toast` 用于 Windows 系统通知。

### 运行命令
在项目根目录执行：

```bash
python study_game_pro.py
```

## 2. 第一次使用注意事项

- **首次启动必须选择一个“数据存储根目录”**，未选择会直接退出。
- 程序会在该根目录下创建/使用：
  - `专注改变（个人软件数据）`
- **所有数据都存放在该文件夹内**：
  - `study_game_reward.json`
  - `daily_tasks_log.txt`
  - `随手记.txt`
  - `随手记/`（图片与附件按日期归档）
- 如果需要更改数据存储路径，可在软件右侧点击“更改数据存储目录”。
- 软件支持防多开，同一时间只允许一个实例运行。

## 功能特性
- **番茄钟专注管理**: 集成经典番茄工作法，针对“科研”、“理论/技术”类别进行专注度追踪。
- **自定义打卡任务**: 每日动态任务看板，并且针对“生活”、“兴趣爱好”提供弹窗手动记录时间的功能。
- **长期任务系统**: 增加动态注入的长期目标（例如：坚持30天的“单词100”任务），带有每日目标耗时强制要求。
  - 长期任务会每日自动注入到当日清单中，任务名格式为：`（长期）任务名（MMDD）`。
  - 若某天的长期任务未完成，会跨天携带到次日；因此你可能会看到“昨天未完成 + 今天新注入”的长期任务同时存在（用于变相强制完成、养成习惯）。
- **数据报表与导出**: 丰富的数据图表统计分析、每日专注与兑换折线图，生成详细的历史记录 CSV 报表（基于时间切割）。
  - 勾选/取消勾选任务会实时刷新导出的 CSV 报表。
  - 取消勾选【科研/理论】只撤销报表中的最新一条记录，不删除内部历史；重新勾选可恢复到报表。
  - 取消勾选【生活/兴趣爱好】会删除该任务最新一条专注记录，避免重复勾选出现时间冲突。
  - Windows 下如果用 Excel/WPS 打开了 CSV，会导致文件被占用而无法实时写入；关闭文件后再操作一次即可刷新。
- **游戏化体验与积分兑换**: 用学习换积分，用积分兑换自己定下的“娱乐时间”。
- **强大的总结与归纳**: 每日强制工作日志、睡前零点前复盘强提醒、以及全格式无缝拖拽的个人“随手记”系统。

## 2.1 数据目录结构

```
<你选择的根目录>
└─ 专注改变（个人软件数据）
  ├─ study_game_reward.json
  ├─ daily_tasks_log.txt
  ├─ 随手记.txt
  └─ 随手记
    └─ 2026-04-13
      └─ example.png
```

## 3. 主要功能介绍

- **番茄钟专注**：25 分钟专注 + 5 分钟休息，支持任务绑定与更换。
  
  ![选择专注任务](选择专注任务.png)
  ![专注结束强制提示](专注结束强制提示.png)

- **每日任务清单**：按类别管理任务，支持打卡与完成率统计。
  - 未完成任务会自动带入次日，直到完成为止（包含长期任务）。
  - 取消未完成任务需填写原因，并按次数扣分（20/40/80...），每月重置。
  - 在“任务打卡看板”打开的情况下新增长期任务，清单会自动刷新，无需手动关闭再打开。
  
  ![今日打卡看板](今日打卡看板.png)

- **自动奖惩与复盘**：每日完成率结算，记录奖励/惩罚日志。
  
  ![今日复盘](今日复盘.png)

- **数据统计与图表**：学习时间分布、任务占比、奖励曲线等图表。
  
  ![专注可视化](专注可视化.png)

- **游戏兑换系统**：积分兑换游戏时间，带防沉迷与激励池。
  
  ![兑换商店](兑换商店.png)
  ![激励计划](激励计划.png)

- **随手记归档**：文本 + 文件/图片归档，按日期整理。
  
  ![随手记归档面板](随手记归档面板.png)

- **工作日志**：按任务聚合今日专注耗时；每日会自动归档所有任务耗时（含兴趣爱好）。
  
  ![今日工作日志](今日工作日志.png)

## 4. Windows 打包为 .exe

推荐使用 PyInstaller：

### 1) 安装打包工具

```bash
pip install pyinstaller
```

### 2) 打包命令（单文件模式）

在项目根目录执行：

```bash
python -m PyInstaller --onefile --noconsole --name change_self study_game_pro.py
```

参数说明：
- `-F`：打包为单个 exe 文件。
- `-w`：窗口模式（不显示黑色命令行）。
- `-n`：自定义生成的 exe 名称。

### 3) 打包产物

- 生成的 exe 位于 `dist/change_self.exe`。

> 备注：
> - 若打包后运行缺少字体或通知功能，请确保目标系统已安装对应字体或安装可选依赖。
> - 如需更小体积，可尝试去掉不必要的依赖或改为目录模式。
> - 如需中文文件名，可在打包完成后手动重命名 exe。

## 5. 更新日志

- v0.2.2 (2026-04-17)
  - 优化：新增长期任务后，已打开的“任务打卡看板”会自动刷新显示最新任务。

- v0.2.1 (2026-04-17)
  - 修复：在“未触发跨天结算”的场景下，长期任务可能不会自动注入到今日清单的问题。
  - 行为说明：未完成的长期任务会跨天携带，叠加到次日的新注入长期任务上。

- v0.2.0 (2026-04-16)
  - 新增“长期任务”创建与每日动态打卡下发功能。
  - 新增并完善历史学习数据与专注记录的 CSV 报表导出。
  - 任务打卡看板：勾选/取消勾选实时刷新报表；取消勾选仅从报表撤销“最新一条”记录（不删除历史），重新勾选可恢复。
  - 修复报表导出字段不一致导致的“报表为空”。
- v0.1.0 (2026-04-07)
  - 初始版本：番茄钟、任务清单、复盘、统计图表、随手记、兑换系统。

## 6. License

本项目采用 MIT 许可证，详见 [LICENSE](LICENSE)。
